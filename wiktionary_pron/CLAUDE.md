...
- **Corpus read-through audit (M-018) + 4-expert panel (M-019) + architect re-audit advisory (M-020, 2026-08-08/09)** fixed 8 systematic classes + homograph/comprehension defects. Read L&S in bulk (`lemma|raw|artifact` TSV), not grep. Key fixes in `build_glosses.cjs`: `de Or` strip needs `\bde`+separator; isGlossRun capital-accept checks ORIGINAL-case token + excludes etymology fragments ("Erse, aile"); scoreGloss shape tests/tail penalties on paren-stripped string; article-Latin-quote gate; text-critical gate; **isSpurious uses the DUAL signature — form+accent AND form+tag must BOTH match the bare twin** (form+accent alone regressed testis2 "testicle"→"witness"; form+tag alone regressed levo2; the intersection captures every signal; golden-runner cache stores both); cross-ref recursion handles fr./from with a "Part." guard + POS-skip; unassimilated-prefix compound table (adfacio→afficio); opp./v.X tail strips; ?-gloss acceptance; wAdjFirst + WORDS-first fallbacks. ~90 unresolvable homographs are byte-identical paradigm duplicates (no signal; L&S numbers primary -1, so the numbered -2 is often the COMMON sense — osculo2 "kiss") — curate by dictionary judgement ~17 done. Current: core **1887**, golden **1960**, census **348**, artifact **33,957 lemmas / 472 KB**, L&S 83.3% / WORDS 9.2% / none 7.6%. Full detail: `docs/ISSUES.md` M-018/M-019/M-020 + `docs/LESSONS.md`.
