...
- **Corpus read-through audit (M-018) + 4-expert panel (M-019, 2026-08-08)** fixed 8 systematic classes + homograph/converage defects. Read L&S in bulk (`lemma|raw|artifact` TSV), not grep. Key fixes in `build_glosses.cjs`: `de Or` strip needs `\bde`+separator; isGlossRun capital-accept checks ORIGINAL-case token + excludes etymology fragments ("Erse, aile"); scoreGloss shape tests/tail penalties on paren-stripped string; article-Latin-quote gate; text-critical gate; **isSpurious uses form+ACCENT signature** (distinct vowel length = distinct homograph: levo2 "to smooth" ≠ levo "to lift" — form+tag conflated them); cross-ref recursion handles fr./from with a "Part." guard ("locative from is" is etymology) + POS-skip ("v. a. caedo"→caedo); unassimilated-prefix compound table (adfacio→afficio); opp./v.X tail strips; ?-gloss acceptance; wAdjFirst + WORDS-first fallbacks. Current: core **1856**, golden **1941**, census **348**, artifact **33,958 lemmas / 472 KB**, L&S 83.3% / WORDS 9.2% / none 7.6%. Full detail: `docs/ISSUES.md` M-018/M-019 + `docs/LESSONS.md`.
