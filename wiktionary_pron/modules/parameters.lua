...
local max = math.max
local pairs = pairs
local pattern_escape = require_when_needed("string/pattern_escape")
local remove_holes = require_when_needed("parameters/remove holes")
local scribunto_param_key = require_when_needed("utilities/scribunto parameter key")
local sort = table.sort
local trim = mw.text.trim
local type = type
local yesno = require_when_needed("yesno")
...
local get_val = setmetatable({
	...
	["family"] = function(val, name, param)
		...
		return require("families")[param.method == "name" and "getByCanonicalName" or "getByCode"](val) or
	...
	["language"] = function(val, name, param)
		...
		local lang = require("languages")[param.method == "name" and "getByCanonicalName" or "getByCode"](val, nil, param.etym_lang, param.family)
		if lang then
			return lang
		end
		local list = {"language"}
	...
	["script"] = function(val, name, param)
		...
		return require("scripts")[param.method == "name" and "getByCanonicalName" or "getByCode"](val) or
	...
	["wikimedia language"] = function(val, name, param)
		...
		return require("wikimedia languages").getByCode(val) or
